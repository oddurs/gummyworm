...
**Output** (a 600×600 source, narrowed to 60 columns to fit this page):
...
The source is square, and so is the result: 60 columns by 30 rows, which at a
2:1 character cell is as wide as it is tall. See
[`--char-aspect`](cli-reference.md#--char-aspect) if it does not look square in
your terminal.
